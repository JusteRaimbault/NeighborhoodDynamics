...
to setup-globals
  ...
  set beta-stay 1000
  set beta-gain 1
  set beta-move 10 ^ log-beta-move
  ; set winc 5
end

to setup-households
  ask patches [
    sprout-households #-households / count patches [
      new-household   
    ]
  ]
end
...
to go
  ...
  if ticks = total-time-steps [stop]
...
to move-households
  ; TODO ALSO tirage avec remise / sans remise may influence final result ?
  
  ask households [
    set moved? false
    ; Q : does noetlogo optimize code : agents -> vectorial computation ? could we imagine an extension doing that ? what would be the generic conditions for that ?
    if random-float 1 > pstay [ ; moves
      ; select one random destination and computes pmove
      let potential-destination one-of other patches
...
to update-households-wealth
  ask households [
    if not moved? [
      set wealth wealth * (1 + (winc / 100))
    ] 
  ]
end
...
to update-patch-vars
  ask patches [
    set population count households-here
    set neigh-wealth sum [wealth] of households-here 
